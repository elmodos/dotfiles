...
--
-- Leader -> "f"        label every clickable element in the focused window,
--                      type the label to LEFT-click it.
-- Leader -> shift+"f"  same, but RIGHT-click the chosen element.
...
return function(leader)
    ...
    -- ---- Action ------------------------------------------------------------
    local function clickElement(frame)
    ...
    -- ---- Entry -------------------------------------------------------------
    local function start(act)
        cleanup()
        action = act

        local win = hs.window.focusedWindow()
        if not win then leader.alert("No focused window"); return end
        local axwin = hs.axuielement.windowElement(win)
        if not axwin then leader.alert("No accessibility for this window"); return end

        local sf = win:screen():fullFrame()
        local acc = {}
        collect(axwin, 0, acc, sf)
        if #acc == 0 then leader.alert("No clickable elements found"); return end
    ...
    leader.registerCommand({}, "f", function() start("left") end, "f — click")
    leader.registerCommand({ "shift" }, "f", function() start("right") end, "⇧f — right-click")
end
